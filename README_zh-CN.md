...
从 Releases 页面下载 `genspark2api-v1.0.1.zip`，解压到一个**单独的文件夹**（比如 `C:\tools\genspark2api`）：
...
| 页面 | 功能 |
|---|---|
| **仪表盘** | 查看账号状态（成功数、失败数、冷却时间）、最近请求记录 |
| **账号管理** | 添加/删除账号、一键获取 Cookie、强制冷却、重置状态 |
| **API 密钥** | 生成新密钥、删除旧密钥（至少保留一把）、脱敏列表 |
| **设置** | 修改管理员密码 |
